...
export async function POST(
  ...
  if (productRequest.userId !== user.id) {
    return NextResponse.json({ error: "You can only boost your own request" }, { status: 403 });
  }
  if (productRequest.status !== "OPEN") {
    return NextResponse.json({ error: "Only open requests can be boosted" }, { status: 400 });
  }
  if (productRequest.promotions.length > 0) {
    return NextResponse.json(
      { error: "This request already has an active promotion request" },
      { status: 409 },
    );
  }
