...
> = {
  PENDING_REVIEW: {
    ...
    message:
      "Your promotion is safely in the review queue. An administrator will check the campaign details shortly.",
    color:
  ...
  PROCESSING: {
    label: "Processing",
    message:
      "The BeforeSell team is preparing your promotion and checking placement availability.",
  ...
  REJECTED: {
    label: "Needs changes",
    message:
      "This request needs an update before it can run. Review the note below and submit a new campaign.",
...
export function BoostListingForm({
  ...
  const activePromotion =
    submittedPromotion?.status !== "REJECTED" ? submittedPromotion : null;
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.success("Campaign saved. Complete your bKash payment next.");
      router.push(`/promotions/${result.promotion.id}/payment`);
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Could not submit promotion",
      );
    } finally {
  ...
  if (activePromotion) {
    ...
    const awaitingPayment = activePromotion.payment?.status === "PENDING";
    return (
      <main className="container max-w-2xl px-4 py-8 sm:py-12">
        <Card>
            ...
            <div className="space-y-2">
              ...
              <CardTitle className="text-3xl">
                {awaitingPayment
                  ...
                  : "Your boost is in motion"}
              </CardTitle>
              <CardDescription className="max-w-xl text-base leading-7">
                {awaitingPayment
                  ? "Your campaign is saved. Submit your bKash transaction ID when you are ready."
                  : current.message}
              </CardDescription>
            </div>
          </CardHeader>
          <CardContent className="space-y-6">
            <Badge
              variant="outline"
              className={cn("gap-2 px-3 py-1.5", current.color)}
            >
              <span className="h-2 w-2 bg-current" />
              {awaitingPayment ? "Payment pending" : current.label}
            </Badge>
            ...
            <div className="grid border-y sm:grid-cols-3">
              ...
              <div className="border-b py-4 sm:border-b-0 sm:border-r sm:px-4">
                ...
                <p className="mt-1 font-semibold">
                  {activePromotion.durationDays} days
                </p>
              </div>
              <div className="py-4 sm:pl-4">
                ...
                <p className="mt-1 font-semibold">
                  ৳{formatNumber(activePromotion.totalBudget)}
                </p>
              </div>
            </div>
          ...
          <CardFooter className="flex-col gap-3 sm:flex-row">
            {activePromotion.payment ? (
              <Button asChild size="lg">
                <Link href={`/promotions/${activePromotion.id}/payment`}>
                  {activePromotion.payment.status === "APPROVED"
                    ? "View payment"
                    : "Complete payment"}
                </Link>
              </Button>
            ...
            {submittedPromotion?.status === "REJECTED" ? (
              <div className="border border-destructive/30 bg-destructive/10 p-4 text-destructive">
                <p className="font-semibold">
                  Your previous request needs changes
                </p>
                <p className="mt-1 text-sm leading-6">
                  {submittedPromotion.reviewNote ||
                    "Please review your campaign details and submit again."}
                </p>
              </div>
            ) : null}
              ...
              <CardContent className="space-y-4">
                ...
                >
                  <span>
                    ...
                    <span className="mt-1 block text-sm leading-6 text-muted-foreground">
                      BeforeSell automatically finds people likely to be
                      interested.
                    </span>
                  </span>
                  <span
                    className={cn(
                      "mt-1 flex h-5 w-5 items-center justify-center border",
                      audienceType === "AUTOMATIC"
                ...
                >
                  <span>
                    ...
                    <span className="mt-1 block text-sm leading-6 text-muted-foreground">
                      Set a location and age range manually.
                    </span>
                  </span>
                  <span
                    className={cn(
                ...
                <div className="grid gap-4 border-t pt-5 sm:grid-cols-2 lg:grid-cols-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      disabled={audienceType === "AUTOMATIC"}
                    />
                  </div>
                  <div className="space-y-2">
              ...
              <CardHeader>
                <div className="flex items-start gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <CardDescription>
                      Control when your campaign runs and how much you spend.
                    </CardDescription>
                  </div>
                </div>
              </CardHeader>
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex h-9 border border-input">
                      <button
                        type="button"
                        className="w-10 text-xl text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground"
                        onClick={() =>
                          setDurationDays((value) => Math.max(1, value - 1))
                        }
                      ...
                      <Input
                        id="duration"
                        className="h-full border-y-0 text-center"
                        type="number"
                        min={1}
                        max={30}
                        value={durationDays}
                      ...
                      <button
                        ...
                        className="w-10 text-xl text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground"
                        onClick={() =>
                          setDurationDays((value) => Math.min(30, value + 1))
                        }
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex h-9 items-center border border-input bg-muted px-3 text-sm text-muted-foreground">
                      {endDate.toLocaleDateString("en-BD", {
                        day: "numeric",
                        month: "short",
                        year: "numeric",
                      })}
                ...
                <div>
                  ...
                  <div className="mt-5 border-l-4 border-primary bg-primary/5 px-4 py-3">
                    <p className="text-sm">
                      Estimated{" "}
                      <strong>
                        {formatNumber(estimatedMinReach)}–
                        {formatNumber(estimatedMaxReach)} views per day
                      </strong>{" "}
              ...
              <CardContent>
                <div className="overflow-hidden border bg-card">
                  <div className="flex items-center gap-3 p-3">
                    <div className="flex h-9 w-9 items-center justify-center bg-primary font-bold text-primary-foreground">
                      B
                    </div>
                    <div>
                      <p className="text-sm font-bold">BeforeSell</p>
              ...
              <CardContent>
                <dl className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between gap-4">
                    ...
                    <dd className="font-semibold">{durationDays} days</dd>
                  </div>
                  <div className="flex justify-between gap-4">
                  ...
                  <div className="border-t pt-4">
                    <div className="flex items-end justify-between">
                      <dt className="font-semibold">Total budget</dt>
                      <dd className="text-2xl font-bold text-primary">
                        ৳{formatNumber(totalBudget)}
                      </dd>
                    </div>
                    {/* <p className="mt-1 text-right text-xs text-muted-foreground">
                      Pay through bKash on the next page
                    </p> */}
                  </div>
                </dl>
                <Button
                ...
                >
                  ...
                  {isSubmitting ? "Saving…" : "Pay now"}
                  {isSubmitting ? null : <ChevronRight className="ml-auto" />}
                </Button>
                <p className="mt-4 text-center text-xs leading-5 text-muted-foreground">
                  Your campaign will be saved as pending so you can complete
                  payment later.
                </p>
              </CardContent>
            </Card>
